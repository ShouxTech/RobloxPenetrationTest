...
void DrawString(std::string str, float fontSize, float x, float y, float r, float g, float b, float a) {
	...
	if (SUCCEEDED(res)) {
		...
		HRESULT metricsResult = w_layout->GetMetrics(&textMetrics);
		DWRITE_TEXT_RANGE range = { 0, str.length() };
		w_layout->SetFontSize(fontSize, range);
		solid_brush->SetColor(D2D1::ColorF(r, g, b, a));
		if (SUCCEEDED(metricsResult)) {
			target->DrawTextLayout(D2D1::Point2F(x - (textMetrics.widthIncludingTrailingWhitespace / 2), y), w_layout, solid_brush);
		}
		w_layout->Release();
		w_layout = NULL;
	}
...
void d2oSetup(HWND tWindow) {
	...
	overlayWindow = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST,
		wc.lpszClassName, (LPCWSTR)Utility::CreateRandomString(6).c_str(), WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
